...
class RotaryEmbeddings(Module):
    ...
    def forward(self, input: torch.Tensor, *, positions: Optional[Tensor] = None, num_heads: int = None):
        """
        ...
        """
        input = input.view(-1, num_heads, input.shape[-2], input.shape[-1])
    ...
    def forward(
        ...
        *,
        ...
        if rotary_dims == dims_per_head:
            ...
            key = self.rotary_embeds(key, positions=positions, num_heads=num_kv)
            query = self.rotary_embeds(query, positions=positions, num_heads=num_heads)
        else:
            ...
            k_rotary = self.rotary_embeds(k_rotary, positions=positions, num_heads=num_kv)
            q_rotary = self.rotary_embeds(q_rotary, positions=positions, num_heads=num_heads)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        query_layer, key_layer = self.rotary_embeds(
                query=query_layer, key=key_layer, cache=layer_past, positions=positions, num_heads=self.num_heads, num_kv=self.num_kv
            ) #self.maybe_rotary(query_layer, key_layer)

        if layer_past is not None:
            past_key, past_value = layer_past
            past_key = past_key.view(batch_size, self.num_kv, -1, self.head_dim)
            past_value = past_value.view(batch_size, self.num_kv, -1, self.head_dim)
